...
public class EmployeeDatabase<T> {
    ...
    public void addEmployee(Employee<T> employee) {
        ...
        if (employees.containsKey(employee.getEmployeeId())) {
            LOGGER.log(Level.WARNING, "Employee with ID {0} already exists", employee.getEmployeeId());
    ...
    public Employee<T> getEmployee(T employeeId) throws EmployeeNotFoundException {
        ...
        if (employee == null) {
            LOGGER.log(Level.WARNING, "Employee with ID {0} not found", employeeId);
    ...
    public void removeEmployee(T employeeId) throws EmployeeNotFoundException {
        ...
        if (employee == null) {
            LOGGER.log(Level.WARNING, "Attempted to remove non-existent employee with ID: {0}", employeeId);
            throw new EmployeeNotFoundException("Employee with ID " + employeeId + " not found");
        }
    ...
    public List<Employee<T>> getHighPerformingEmployees(double minRating) {
        if (minRating < 0 || minRating > 5) {
            LOGGER.log(Level.WARNING, "Invalid minimum rating: {0}", minRating);
            throw new IllegalArgumentException("Rating must be between 0 and 5");
        }
        List<Employee<T>> result = employeeList.stream()
    ...
    public List<Employee<T>> getEmployeesInSalaryRange(double minSalary, double maxSalary) 
        ...
        if (minSalary > maxSalary) {
            LOGGER.log(Level.WARNING, "Invalid salary range: min={0}, max={1}", 
                new Object[]{minSalary, maxSalary});
    ...
    public List<Employee<T>> getTopPaidEmployees(int count) {
        if (count <= 0) {
            LOGGER.log(Level.WARNING, "Invalid count for top paid employees: {0}", count);
    ...
    public double getAverageDepartmentSalary(String department) throws InvalidDepartmentException {
        ...
        LOGGER.log(Level.INFO, "Calculated average salary for department {0}: {1}", 
