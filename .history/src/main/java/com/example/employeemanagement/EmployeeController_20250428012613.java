...
public class EmployeeController {
    ...
    private final List<String> departments = Arrays.asList("HR", "IT", "Finance", "Marketing", "Operations", "Sales");
    ...
    private void setupComboBoxes() {
        try {
            ...
            // Setup department analytics ComboBox
            departmentAnalyticsField.setItems(FXCollections.observableArrayList(departments));
    ...
    private void setupValidation() {
        try {
            ...
            ratingField.valueProperty().addListener((obs, oldVal, newVal) -> validateRating(newVal));
    ...
    private void validateRating(Integer value) {
        if (value == null) {
            showAlert("Validation Error", "Rating cannot be empty");
        }
    }
    ...
    private void validateExperience(String value) {
        try {
            if (value == null || value.trim().isEmpty()) {
                showAlert("Validation Error", "Experience cannot be empty");
                return;
            }
    ...
    private void handleAddEmployee() {
        try {
            ...
            int rating = ratingField.getValue();
            ...
            showAlert("Invalid Input", "Please enter valid numeric values for salary and experience.");
        } catch (Exception e) {
    ...
    private void handleUpdateEmployee() {
        ...
        if (selectedEmployee != null) {
            try {
                ...
                database.updateEmployeeDetails(selectedEmployee.getEmployeeId(), "performanceRating", rating);
                database.updateEmployeeDetails(selectedEmployee.getEmployeeId(), "yearsOfExperience", experience);
    ...
    private void handleSearch() {
        ...
        employeeList.clear();
        employeeList.addAll(database.getAllEmployees().stream()
            .filter(emp -> emp.getName().toLowerCase().contains(searchText))
            .filter(emp -> selectedDepartment.equals("All Departments") || emp.getDepartment().equals(selectedDepartment))
            .toList());
    }
